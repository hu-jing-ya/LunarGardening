...
vnMoon() = 1000(40 - sqrt(1000rand(Float64))) * sin(pi * rand(Float64) / 2)
randvn = vnMoon
...
plot(histgen(0, 0.9, expdist, 400)[[1, 2]])


for i in 1:100

hist=histgen(0.25, 0.9, expdist, 400)

ind_compSm = compSm(hist, sm_npr_list[4], abunSm0, 2.5)
ind_compGd = compGd(hist, gd_npr_list[4], abunGd0, 2.5)
...
CSV.write("$i.csv", DataFrame([hist..., dSm,dGd], :auto))

sum(hist[2].>5)>0&&break
